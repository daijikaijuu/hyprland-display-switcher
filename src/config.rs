...
impl Default for DisplayConfig {
    fn default() -> Self {
        Self {
            extend_configurations: HashMap::new(),
            last_used_extend_config: None,
        }
    }
}
...
impl ConfigManager {
    ...
    pub fn save_extend_configuration(&mut self, config: ExtendConfiguration) -> anyhow::Result<()> {
        ...
        self.config.extend_configurations.insert(key.clone(), updated_config);
    ...
    pub fn get_last_extend_configuration(&self) -> Option<&ExtendConfiguration> {
        self.config.last_used_extend_config
            .as_ref()
    ...
    pub fn get_extend_configuration_by_monitors(&self, primary: &str, secondary: &str) -> Option<&ExtendConfiguration> {
    ...
    pub fn get_extend_configuration_for_monitors(&self, available_monitors: &[String]) -> Option<&ExtendConfiguration> {
        // Find the most recent configuration that uses any of the available monitors
        let mut matching_configs: Vec<_> = self.config.extend_configurations.values()
            .filter(|config| {
